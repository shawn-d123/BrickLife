...
the game via `web/src/data/predictions.json` and `web/public/backtest.png`. Built
and documented by Hemakesh Bavuluru.
...
python 02_baselines.py         # B0/B1/B2 baseline console print
...
## `predictions.json` notes, for the in-game "what's real" panel
...
- `meta.scenario_method`: the rate scenarios shift Bank Rate by the stated
  ...
  stays consistent with the band. The engine separately applies `rate_delta_pp`
  directly to the player's own mortgage rate — see the root README's
